...
void NaClStraceClockGetCommon(int cageid, int clk_id, uint32_t ts_addr, void *time_func, int ret) {
...
void NaClStraceMmapIntern(int cageid, uintptr_t start, size_t length, int prot, int flags, int d, nacl_abi_off_t offset, int32_t retval) {
    fprintf(tracingOutputFile, "%d MmapIntern(addr=0x%08x, length=%zu, prot=%d, flags=%d, fd=%d, offset=%ld) = %d\n",
            cageid, start, length, prot, flags, d, offset, retval);
}