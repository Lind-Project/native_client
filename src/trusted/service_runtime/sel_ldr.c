...
void  NaClLoadTrampoline(struct NaClApp *nap) {
  ...
  for (int i = 0; i < 16; i++) { // install all restore functions for offset 0 -15
      int patch_location = NACL_sys_reg_restore + (i * 4);
      NaClPatchRegTrampolineCall(nap->mem_start + NACL_SYSCALL_START_ADDR + NACL_SYSCALL_BLOCK_SIZE * patch_location); // each of these takes up 4 trampoline slots
  }
