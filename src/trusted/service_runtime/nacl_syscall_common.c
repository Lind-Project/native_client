                     ...
                     struct nacl_abi_stat *nasp)
{
  ...
  if (!retval)
  {
    if (!NaClCopyOutToUser(nap, (uintptr_t)nasp, &result, sizeof(result)))
      return -NACL_ABI_EFAULT;
  }
                     ...
                     void *shmaddr)
{
  ...
  NaClLog(2, "Entered NaClSysShmdt(0x%08" NACL_PRIxPTR " ,"
             "0x%08" NACL_PRIxPTR ")\n",
                      ...
                      struct lind_shmid_ds *buf)
{
  ...
  NaClLog(2, "Entered NaClSysShmctl(0x%08" NACL_PRIxPTR " , %d, %d ,"
             "%p" NACL_PRIxPTR ")\n",
  ...
  if (cmd == IPC_RMID)
  {
    ...
    if (!shmtable[shmid].count)
      clear_shmentry(shmid); // if we dont have any entries attached we can clear it now
  }
                       ...
                       uint32_t thread_ptr)
{
   ...
   */
  sys_tls = NaClUserToSysAddrRangeProt(natp->nap, thread_ptr, 4, NACL_ABI_PROT_WRITE | NACL_ABI_PROT_READ);
  NaClLog(4,
                            ...
                            uint32_t second_thread_ptr)
{
  ...
  sys_stack = NaClUserToSysAddrProt(nap, stack_ptr, NACL_ABI_PROT_READ | NACL_ABI_PROT_WRITE);
  ...
  sys_tls = NaClUserToSysAddrRangeProt(nap, thread_ptr, 4, NACL_ABI_PROT_READ | NACL_ABI_PROT_WRITE);
  if (kNaClBadAddress == sys_tls)
  {
...
int32_t NaClSysPipe2(struct NaClAppThread *natp, uint32_t *pipedes, int flags)
{
  ...
  /* copy out NaCl fds */
  if (!NaClCopyOutToUser(nap, (uintptr_t)pipedes, lindfds, sizeof(lindfds)))
    return -NACL_ABI_EFAULT;
...
int32_t NaClSysFork(struct NaClAppThread *natp)
{
  ...
  for (int i = 0; i < nap_child->argc; i++)
    nap_child->argv[i] = strdup(nap->argv[i]);
                       ...
                       int options)
{
  ...
  if (pid <= 0)
  {
    ...
    if (!zombie && (options & WNOHANG))
      goto out; // exit here if WNOHANG, ret stays initialized to 0
    while (!zombie)
    {
