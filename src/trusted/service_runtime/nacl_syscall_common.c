                   ...
                   uintptr_t new_break)
{
  ...
  else
  {
    ...
    if (!NaClVmmapFindPageIter(&nap->mem_map, usr_last_data_page, &iter) || NaClVmmapIterAtEnd(&iter))
    {
    ...
    else
    {
      ...
      if (!NaClVmmapIterAtEnd(&iter) && ((next_ent = NaClVmmapIterStar(&iter))->page_num <= last_internal_page))
      {
      ...
      region_size = (((last_internal_page + 1) << NACL_PAGESHIFT) - start_new_region);
                    ...
                    int mode)
{
  ...
  allowed_flags = (NACL_ABI_O_ACCMODE | NACL_ABI_O_CREAT | NACL_ABI_O_TRUNC | NACL_ABI_O_APPEND);
                     ...
                     int whence)
{
  ...
  retval64 = (*((struct NaClDescVtbl const *)ndp->base.vtbl)->Seek)(ndp, (nacl_off64_t)offset, whence);
                     ...
                     void *arg)
{
  ...
  NaClLog(2, "Entered NaClSysIoctl(0x%08" NACL_PRIxPTR ", %d, %d, 0x%08" NACL_PRIxPTR ")\n",
   ...
   */
  NaClXMutexLock(&nap->mu);
  retval = (*((struct NaClDescVtbl const *)ndp->base.vtbl)->Ioctl)(ndp, request, (void *)sysaddr);
  NaClXMutexUnlock(&nap->mu);
cleanup_unref:
  NaClDescUnref(ndp);
cleanup:
                     ...
                     struct nacl_abi_stat *nasp)
{
  ...
  retval = lind_fxstat(lind_fd, nasp, nap->cage_id))
                          ...
                          nacl_abi_off_t offset)
{
  ...
  allowed_flags = (NACL_ABI_MAP_FIXED | NACL_ABI_MAP_SHARED | NACL_ABI_MAP_PRIVATE | NACL_ABI_MAP_ANONYMOUS);
  ...
  if (length > 0)
  {
    ...
    else if (mapping_code)
    {
      ...
      image_sys_addr = NACL_VTBL(NaClDesc, ndp)->Map(ndp, NaClDescEffectorTrustedMem(), NULL, length, NACL_ABI_PROT_READ | NACL_ABI_PROT_WRITE, NACL_ABI_MAP_PRIVATE, offset);
      ...
      validator_status = NACL_FI("MMAP_FORCE_MMAP_VALIDATION_FAIL",
                                 (*nap->validator->Validate)(usraddr,
...
#if NACL_WINDOWS
      sys_ret = (*NACL_VTBL(NaClDesc, ndp)->UnmapUnsafe)(ndp, (void *)image_sys_addr, length);
#else
    ...
    else
    {
      ...
      map_result = (*((struct NaClDescVtbl const *)ndp->base.vtbl)->Map)(ndp,
                                                                         nap->effp,
                                                                         (void *)sysaddr,
                                                                         length,
                                                                         prot,
...
cleanup_no_locks:
   ...
   */
  if (map_result > UINT32_MAX && !NaClPtrIsNegErrno(&map_result))
  {
  ...
  for (addr = sysaddr; addr < endaddr; addr += NACL_MAP_PAGESIZE)
  {
    ...
    else if (offset < (uintptr_t)entry->file_size)
    {
      ...
      NaClflProtectAndDesiredAccessMap(prot,
                                       (entry->flags & NACL_ABI_MAP_PRIVATE) != 0,
                          ...
                          int flags)
{
  ...
  ssize_retval = NACL_VTBL(NaClDesc, ndp)->RecvMsg(ndp, &recv_hdr, flags, (struct NaClDescQuotaInterface *)nap->reverse_quota_interface);
  /* unlock user memory ranges in kern_naiov */
  for (i = 0; i < kern_nanimh.iov_length; ++i)
  {
    NaClVmIoHasEnded(nap,
  ...
  {
    NaClLog(1, "NaClSysImcRecvMsg: in/out ptr (descv %" NACL_PRIxPTR ") became invalid at copyout?\n",
                               ...
                               size_t size)
{
  ...
  NaClLog(2, "Entered NaClSysImcMemObjCreate(0x%08" NACL_PRIxPTR " 0x%08" NACL_PRIxS ")\n",
          (uintptr_t)natp, size);
                             ...
                             uint32_t descs_out)
{
  ...
  NaClLog(2, "Entered NaClSysImcSocketPair(0x%08" NACL_PRIxPTR " 0x%08" NACL_PRIx32 ")\n",
                            ...
                            uint32_t second_thread_ptr)
{
  ...
  NaClLog(3,
          ("Entered NaClSysThreadCreate(0x%08" NACL_PRIxPTR
           " pc=0x%08" NACL_PRIxPTR ", sp=0x%08" NACL_PRIx32 ", thread_ptr=0x%08" NACL_PRIx32 ")\n"),
  ...
  stack_ptr = ((stack_ptr + NACL_STACK_PAD_BELOW_ALIGN) & ~NACL_STACK_ALIGN_MASK) - NACL_STACK_PAD_BELOW_ALIGN - NACL_STACK_ARGS_SIZE;
                        ...
                        int32_t mutex_handle)
{
  ...
  retval = (*((struct NaClDescVtbl const *)cv_desc->base.vtbl)->Wait)(cv_desc, mutex_desc);
  NaClDescUnref(cv_desc);
                                ...
                                struct nacl_abi_timespec *ts)
{
  ...
  NaClLog(2, "Entered NaClSysCondTimedWaitAbs(0x%08" NACL_PRIxPTR ", %d, %d, 0x%08" NACL_PRIxPTR ")\n",
                       ...
                       int32_t sem_handle)
{
  ...
  NaClLog(2, "Entered NaClSysSemWait(0x%08" NACL_PRIxPTR ", %d)\n",
                       ...
                       int32_t sem_handle)
{
  ...
  NaClLog(2, "Entered NaClSysSemPost(0x%08" NACL_PRIxPTR ", %d)\n",
          (uintptr_t)natp, sem_handle);
                           ...
                           int32_t sem_handle)
{
  ...
  NaClLog(2, "Entered NaClSysSemGetValue(0x%08" NACL_PRIxPTR ", %d)\n",
          (uintptr_t)natp, sem_handle);
                         ...
                         struct nacl_abi_timespec *rem)
{
  ...
  NaClLog(2, "Entered NaClSysNanosleep(0x%08" NACL_PRIxPTR ", 0x%08" NACL_PRIxPTR ", 0x%08" NACL_PRIxPTR "x)\n",
...
#error Unsupported platform
  ...
  __asm__ volatile("fmxr fpscr, %0" ::"r"(0xdeadbeef)
                   : "vfpcc");
...
int32_t NaClSysExecve(struct NaClAppThread *natp, char const *path, char *const *argv, char *const *envp)
{
  ...
  */
  ...
  if (envp)
    sys_envp_ptr = (uint32_t *)NaClUserToSysAddr(nap, (uintptr_t)envp);
                        ...
                        struct sockaddr *src_addr, socklen_t *addrlen)
{
  ...
  struct NaClApp *nap = natp->nap;
  void *sysbufaddr = (void *)NaClUserToSysAddrRange(nap, (uintptr_t)buf, len);
  socklen_t *sysaddrlenaddr = (void *)NaClUserToSysAddrRange(nap, (uintptr_t)addrlen, sizeof(socklen_t));
  ...
  sysaddraddr = (void *)NaClUserToSysAddrRange(nap, (uintptr_t)src_addr, *sysaddrlenaddr);
